...
| 模式 | 成本 | 启用方式 |
|------|------|---------|
| **Qwen3.5-omni-plus** ⭐ | ~¥0.002/次 | 复制 `.env.example` 为 `.env`，配置 `QWEN_API_KEY` |
| Mock（模拟评分） | 免费 | 默认启用，无需配置 |

> 默认使用 Mock 评分，便于本地演示。如需启用 Qwen 视觉评测，请参考 `api-server/.env.example` 配置 API Key。
