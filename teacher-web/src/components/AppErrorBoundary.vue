...
async function resetError() {
  ...
  // 等待 DOM 更新后再递增 key，确保旧组件树完全卸载后再新建
  await nextTick();
  recoverKey.value++;
}
...
.error-card {
  ...
  box-shadow: 0 18px 40px rgba(58, 35, 17, 0.12);
}
...
.error-btn {
  ...
  border-radius: 14px;
  padding: 12px 18px;
  background: linear-gradient(135deg, #a6482e, #c4663f);
  color: #ffffff;
}
...
.error-btn.secondary {
  ...
  color: #2f241a;
  border: 1px solid rgba(166, 72, 46, 0.14);
}
</style>
